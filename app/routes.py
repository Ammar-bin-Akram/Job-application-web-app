...
@main.route("/get-details", endpoint='get-details')
def get_details():
    return render_template("get-details.html", title="Details")


@main.route("/run-bot", methods=['POST'], endpoint='run-bot')
def run_bot():
...
def job_application(driver, path):
    ...
    # if the next page of application does not require you to submit your resume then, close the application
        ...
        discard_button.click()
        job_status = "Not Applied"
    save_job_to_db(job_url, job_status)
...
def save_job_to_db(link, status):
    new_job = Jobs(job_link=link, job_status=status)
